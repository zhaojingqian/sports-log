...
.chart{height:382px}.chart.short{height:252px}.chart.weekly,.chart.sleep{height:306px}.chart-tip{border-radius:8px;background:rgba(11,20,38,.96);box-shadow:0 18px 48px rgba(15,27,51,.24)}
...
@media(max-width:640px){.main{padding:14px}.title h1{font-size:26px}.card,.panel{padding:14px}.chart{height:304px}.chart.short{height:228px}.chart.weekly,.chart.sleep{height:292px}}
...
function drawWeeks(){
 const weeks=(DATA.summaries.weekly||[]).slice(0,8).reverse(),c=$('#weekChart'),{ctx,w,h}=canvas(c),p={l:46,r:16,t:18,b:34},iw=w-p.l-p.r,ih=h-p.t-p.b,den=Math.max(1,weeks.length-1),pts=[];
 const series=[{key:'distance_km',label:'距离',unit:' km',color:C.blue,fmt:v=>(+v||0).toFixed(1)},{key:'activity_min',label:'时长',unit:' h',color:C.orange,fmt:v=>((+v||0)/60).toFixed(1)},{key:'activities',label:'次数',unit:' 次',color:C.green,fmt:v=>Math.round(+v||0)}];
 ...
 for(let i=0;i<4;i++){const y=p.t+i*ih/3,label=100-i*33;ctx.beginPath();ctx.moveTo(p.l,y);ctx.lineTo(w-p.r,y);ctx.stroke();ctx.fillText(i===0?'峰值':i===3?'低':'',p.l-8,y+4)}
 series.forEach(s=>{const max=Math.max(...weeks.map(wk=>+wk[s.key]||0),1),line=[];weeks.forEach((wk,i)=>{const value=+wk[s.key]||0,x=p.l+i*iw/den,y=h-p.b-(value/max)*ih;line.push({x,y,row:wk,value,max,s})});ctx.strokeStyle=s.color;ctx.lineWidth=2.5;ctx.lineJoin='round';ctx.lineCap='round';ctx.beginPath();line.forEach((pt,i)=>i?ctx.lineTo(pt.x,pt.y):ctx.moveTo(pt.x,pt.y));ctx.stroke();line.forEach(pt=>{ctx.save();ctx.globalAlpha=(pt.row.days&&pt.row.days<7)?0.55:1;ctx.fillStyle='#fff';ctx.strokeStyle=s.color;ctx.lineWidth=2;ctx.beginPath();ctx.arc(pt.x,pt.y,3.4,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.restore()})});
 ctx.fillStyle=C.faint;ctx.font='11px Geist, system-ui';ctx.textAlign='center';weeks.forEach((wk,i)=>{const label=String(wk.key||'').replace(/^\\d{4}-/,'');const x=p.l+i*iw/den;ctx.fillText(label+(wk.days&&wk.days<7?'*':''),x,h-10);pts.push({x,y:h*.46,row:wk})});
...
function drawSleep(){
 const a=days(),c=$('#sleepChart'),{ctx,w,h}=canvas(c),p={l:48,r:14,t:16,b:30},iw=w-p.l-p.r,den=Math.max(1,a.length-1),gap=18,sleepH=(h-p.t-p.b-gap)*.58,recH=(h-p.t-p.b-gap)*.42,recTop=p.t+sleepH+gap,pts=[];
 const sleepSeries=[{key:'light_min',label:'浅睡',color:'#bad6c6'},{key:'deep_min',label:'深睡',color:C.green},{key:'rem_min',label:'REM',color:C.blue},{key:'awake_min',label:'清醒',color:'#f6a6aa'}];
 const recSeries=[{key:'hrv',label:'HRV',color:C.cyan},{key:'rhr',label:'RHR',color:C.orange,invert:true}];
 ctx.clearRect(0,0,w,h);ctx.font='11px Geist, system-ui';ctx.lineCap='round';ctx.lineJoin='round';
 const sleepMax=Math.max(...a.flatMap(d=>sleepSeries.map(s=>+d[s.key]||0)),60);
 [['睡眠结构',p.t,sleepH,sleepMax,v=>(v/60).toFixed(1)+'h'],['恢复趋势',recTop,recH,null,null]].forEach(([label,y0,hh,max,labelFn])=>{ctx.fillStyle=C.muted;ctx.textAlign='left';ctx.font='11px Geist, system-ui';ctx.fillText(label,0,y0+hh/2+4);ctx.strokeStyle=C.grid;ctx.lineWidth=1;for(let i=0;i<3;i++){const y=y0+i*hh/2;ctx.beginPath();ctx.moveTo(p.l,y);ctx.lineTo(w-p.r,y);ctx.stroke()}if(max){ctx.fillStyle=C.faint;ctx.textAlign='right';ctx.font='10px JetBrains Mono, monospace';ctx.fillText(labelFn(max),p.l-8,y0+5)}});
 function lineFor(series,y0,hh,max,valuesFn){series.forEach(s=>{const vals=a.map(d=>+d[s.key]).filter(v=>Number.isFinite(v)&&v>0);if(vals.length<2)return;const lo=max?0:Math.min(...vals),hi=max?max:Math.max(...vals),sp=hi-lo||1;ctx.strokeStyle=s.color;ctx.lineWidth=2.1;ctx.beginPath();let started=false;a.forEach((d,i)=>{const v=+d[s.key];if(!Number.isFinite(v)||v<=0){started=false;return}const raw=(v-lo)/sp,ratio=s.invert?1-raw:raw,x=p.l+i*iw/den,y=y0+hh-ratio*hh;if(started)ctx.lineTo(x,y);else{ctx.moveTo(x,y);started=true}});ctx.stroke()})}
 lineFor(sleepSeries,p.t,sleepH,sleepMax);lineFor(recSeries,recTop,recH,null);
 ctx.fillStyle=C.faint;ctx.font='11px Geist, system-ui';ctx.textAlign='center';for(let i=0;i<Math.min(5,a.length);i++){const idx=Math.round(i*(a.length-1)/Math.max(1,Math.min(5,a.length)-1));ctx.fillText(a[idx].date.slice(5),p.l+idx*iw/den,h-9)}a.forEach((row,i)=>pts.push({x:p.l+i*iw/den,y:h*.45,row}));
 hover(c,pts,p=>`<b>${p.row.date}</b><span><em>总睡眠</em><strong>${p.row.sleep_min?((p.row.sleep_min/60).toFixed(1)+' h'):'--'}</strong></span><span><em>浅/深/REM</em><strong>${fmt(p.row.light_min,'m')} / ${fmt(p.row.deep_min,'m')} / ${fmt(p.row.rem_min,'m')}</strong></span><span><em>清醒</em><strong>${fmt(p.row.awake_min,' min')}</strong></span><span><em>HRV / RHR</em><strong>${fmt(p.row.hrv,' ms')} / ${fmt(p.row.rhr,' bpm')}</strong></span>`);
}
